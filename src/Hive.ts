...
export class Hive {
  ...
  public constructor(roomName: string) {
    ...
    if (!this.cache)
      Memory.cache.hives[this.roomName] = {
        wallsHealth: Math.max(Memory.settings.wallsHealth * 0.0025, 10000),
        cells: {},
        do: { ...BASE_MODE_HIVE },
      };
    ...
    if (storage && storage.isActive()) {
      ...
      this.wallsHealth = Math.max(
        Memory.settings.wallsHealth * 0.0005,
        this.wallsHealth
      );
      if (this.controller.level < 8) {
        // try to develop the hive
        this.resTarget[BOOST_MINERAL.upgrade[2]] = HIVE_MINERAL;
        this.wallsHealthMax = Math.min(
          Memory.settings.wallsHealth * 0.1,
          2000000
        );
      } else {
        this.phase = 2;
  ...
  public get wallsHealth() {
    return this.cache.wallsHealth;
  }

  public set wallsHealth(value) {
