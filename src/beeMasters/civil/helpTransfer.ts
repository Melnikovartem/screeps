...
export class HelpTransferMaster extends SwarmMaster {
  ...
  boosts: Boosts = [{ type: "capacity", lvl: 2 }, { type: "capacity", lvl: 1 }, { type: "capacity", lvl: 0 }];
  get targetBeeCount() {
    if (!this.order)
      return 2;
    if (!this.order.memory.extraInfo)
      this.order.memory.extraInfo = 2;
  ...
  update() {
    super.update();
    if (this.checkBees() && Apiary.intel.getInfo(this.pos.roomName, 20).safePlace && (this.hive.resState[this.res] || 0 > 0))
      this.wish({
        setup: setups.pickup,
        priority: 8,
      });
  ...
  run() {
    ...
    _.forEach(this.activeBees, bee => {
      ...
      let lab = bee.ticksToLive < 50 && bee.pos.roomName === this.hive.roomName
        && bee.boosted && this.hive.cells.lab && this.hive.cells.lab.getUnboostLab(bee.ticksToLive);
      if (lab)
