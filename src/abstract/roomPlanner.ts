...
const SPECIAL_STRUCTURE: { [key in StructureConstant]?: { [level: number]: { amount: number, heal: number } } } = {
  [STRUCTURE_CONTAINER]: { 0: { amount: 5, heal: CONTAINER_HITS }, 1: { amount: 0, heal: CONTAINER_HITS }, 2: { amount: 0, heal: CONTAINER_HITS }, 3: { amount: 5, heal: CONTAINER_HITS }, 4: { amount: 5, heal: CONTAINER_HITS }, 5: { amount: 5, heal: CONTAINER_HITS }, 6: { amount: 5, heal: CONTAINER_HITS }, 7: { amount: 5, heal: CONTAINER_HITS }, 8: { amount: 5, heal: CONTAINER_HITS } },
  [STRUCTURE_ROAD]: { 0: { amount: 2500, heal: ROAD_HITS / 2 }, 1: { amount: 15, heal: ROAD_HITS / 2 }, 2: { amount: 30, heal: ROAD_HITS / 2 }, 3: { amount: 2500, heal: ROAD_HITS / 2 }, 4: { amount: 2500, heal: ROAD_HITS / 2 }, 5: { amount: 2500, heal: ROAD_HITS / 2 }, 6: { amount: 2500, heal: ROAD_HITS / 2 }, 7: { amount: 2500, heal: ROAD_HITS / 2 }, 8: { amount: 2500, heal: ROAD_HITS } },
  [STRUCTURE_WALL]: { 0: { amount: 0, heal: 0 }, 1: { amount: 0, heal: 0 }, 2: { amount: 2500, heal: WALL_HEALTH.small / 4 }, 3: { amount: 2500, heal: WALL_HEALTH.small / 4 }, 4: { amount: 2500, heal: WALL_HEALTH.small / 2 }, 5: { amount: 2500, heal: WALL_HEALTH.small }, 6: { amount: 2500, heal: WALL_HEALTH.big / 2 }, 7: { amount: 2500, heal: WALL_HEALTH.big / 2 }, 8: { amount: 2500, heal: WALL_HEALTH.big } },
  [STRUCTURE_RAMPART]: { 0: { amount: 0, heal: 0 }, 1: { amount: 0, heal: 0 }, 2: { amount: 2500, heal: WALL_HEALTH.small / 4 }, 3: { amount: 2500, heal: WALL_HEALTH.small / 4 }, 4: { amount: 2500, heal: WALL_HEALTH.small / 2 }, 5: { amount: 2500, heal: WALL_HEALTH.small }, 6: { amount: 2500, heal: WALL_HEALTH.big / 2 }, 7: { amount: 2500, heal: WALL_HEALTH.big / 2 }, 8: { amount: 2500, heal: WALL_HEALTH.big } }
}
      ...
      jobs.push({
        ...
        func: () => {
          ...
          if (ans === ERR_FULL || ans === ERR_BUSY)
            return ans;
          if (!f.pos.findInRange(FIND_STRUCTURES, 2).length)
            this.addToPlan(ans, f.pos.roomName, STRUCTURE_CONTAINER, true);
          if (f.pos.roomName === anchor.roomName && f.secondaryColor === COLOR_YELLOW) {
            let poss = f.pos.getOpenPositions(true, 2);
            if (!poss.length)
              return ERR_FULL;
