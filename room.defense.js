  ...
  for (var name in Game.structures) {
    ...
    if (structure.structureType == STRUCTURE_TOWER) {
      ...
      if (structure.id == "610a113a21e8a68645e5fb07") { //this is a hotfix anyway //_.filter(structure.pos.findInRange(FIND_MY_STRUCTURES, 2), {structureType : STRUCTURE_STORAGE}).length) {
        let otherLinks = room.find(FIND_MY_STRUCTURES, {filter: (structureIter) => structureIter.structureType == STRUCTURE_LINK && structureIter != structure && structureIter.store.getFreeCapacity(RESOURCE_ENERGY) > 50 });
